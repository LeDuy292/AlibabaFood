...
const filters = ['Near-Expiry Deals', 'Best Seller', 'Best Price', 'Expiring Soon'];
...
const StarRating = ({ count }) => (
    <div className="star-rating">
        {[1, 2, 3, 4, 5].map(i => (
            <svg key={i} width="18" height="18" viewBox="0 0 24 24" fill={i <= count ? '#f59e0b' : '#d1d5db'} xmlns="http://www.w3.org/2000/svg">
...
const HotDealSection = () => {
    ...
    const filtered = activeFilter === 'Near-Expiry Deals'
        ? deals
    ...
    const handleNext = () => setStartIdx(i => Math.min(filtered.length - CARDS_PER_VIEW, i + 1));
        ...
        <section className="hot-deal-section">
            <div className="container">
                ...
                <div className="hot-deal-filters">
                    {filters.map(f => (
                        <button
                            ...
                            onClick={() => { setActiveFilter(f); setStartIdx(0); }}
                        >
                            {f}
                        </button>
                    ))}
                ...
                <div className="hot-deal-grid">
                    {visible.length > 0 ? visible.map(deal => (
                ...
                <div className="hot-deal-nav">
                    <button className="nav-arrow-btn" onClick={handlePrev} disabled={!canPrev} aria-label="Previous">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                            <path d="M15 18L9 12L15 6" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                    </button>
                    <button className="nav-arrow-btn" onClick={handleNext} disabled={!canNext} aria-label="Next">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
