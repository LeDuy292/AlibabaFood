...
BEGIN
    ...
    BEGIN
        ...
        PRINT 'Đã thêm 10 roll credits cho user dauboqu1ay@gmail.com (User ID: ' + CAST(@user_id AS VARCHAR) + ')';
    END
    ELSE
    BEGIN
        UPDATE roll_credits SET credits = credits + 10, updated_at = GETDATE() WHERE user_id = @user_id;
        PRINT 'Đã cộng thêm 10 roll credits cho user dauboqu1ay@gmail.com (User ID: ' + CAST(@user_id AS VARCHAR) + ')';
    END
END
ELSE
BEGIN
    PRINT 'Không tìm thấy user dauboqu1ay@gmail.com trong database';
END
