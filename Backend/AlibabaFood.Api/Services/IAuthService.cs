...
namespace AlibabaFood.Api.Services
{
    public interface IAuthService
    {
        ...
        Task<AuthResponse> LoginWithGoogleAsync(GoogleLoginRequest request, string? ipAddress = null, string? userAgent = null);
        Task<AuthResponse> RegisterAsync(RegisterRequest request, string? ipAddress = null, string? userAgent = null);
        Task<bool> ValidateTokenAsync(string token);
        Task<User?> GetUserByTokenAsync(string token);
        Task<UserDto?> UpdateProfileAsync(string token, UpdateProfileRequest request);
